  ...
  methods: {
    ...
    bodyTouchMove(event) {
      if (this.backBtn && this.touchStartPoint < this.touchLeft) {
        ...
        if (this.distance < 0) {
          // driver组件是右滑，所以totalDiff不能小于0
          this.distance = 0;
        } else if (this.distance > swidth) {
          // 这里maxMoveDistance为屏幕宽度
          this.distance = swidth;
        }
        let el = document.getElementsByClassName("container-view")[
          this.backBtn - 1
        ];
    ...
    translate(el, x, y, options) {
      ...
      if (defaultOptions.useTransfrom) {
        ...
        el.style.transitionTimingFunction =
          defaultOptions.transitionTimingFunction;
